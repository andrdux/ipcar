...
class DBI {
    ...
    function query($sql) {
        ...
        if ($result) {
            ...
            $this->logger(mysql_error($this->link), "failed_queries");
            return FALSE;
        }
    }
     ...
     * @return mixed
     */
    function fetch($result, $rtype = "row", $offset = 0, $classname = "stdClass") {
        $rtype = strtolower($rtype);

        switch ($this->type) {
            case "mysql":
                if ($rtype == "object") {
                    $data = @mysql_fetch_object($result, $classname);
                } else if ($rtype == "field") {
     ...
     * @return mixed
     */
    function fetch_row($sql, $rtype = "row", $classname = "stdClass") {
        $result = $this->query($sql);
        if ($this->num_rows($result) > 0) {
            $row = $this->fetch($result, $rtype, 0, $classname);
            $this->free($result);
            return $row;
        }
    ...
    /**
     * Retrieve the first row from result as object
     * @param string $sql
     * @param string $classname
     * @return object
     */
    function fetch_object($sql, $classname = "stdClass") {
        $result = $this->fetch_row($sql, "object", $classname);
        return $result;
    }

    /**
     * Retrieve all rows from result as object collection
     * @param string $sql
     * @param string $classname
     * @return object 
     */
    function fetch_object_collection($sql, $classname = "stdClass") {
        $result = $this->fetch_all($sql, "object", $classname);
        return $result;
    }
     ...
     * @return array
     */
    function fetch_all($sql, $rtype = "row", $classname = "stdClass") {
        $result = $this->query($sql);
        if ($this->num_rows($result) > 0) {
            $rows = array();

            while ($row = $this->fetch($result, $rtype, 0, $classname)) {
                if ($rtype == "object") {
                    array_push($rows, $row);
                } else {
                    if (count($row) > 1) {
